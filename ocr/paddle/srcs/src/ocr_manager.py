...
def _dummy_get_font_file_path(filename: str) -> str:
    # Option A: return a real existing file (e.g. a blank TTF under /app/paddlex-cache/fonts/)
    # return str((pathlib.Path("/app/paddlex-cache/fonts") / filename).resolve())

    # Option B: return an empty string or some harmless path—OCR itself does not need to actually load this
    return ""

# 2) Replace the paddlex utility with our dummy
import paddlex.utils.fonts as _fonts_mod
_fonts_mod.get_font_file_path = _dummy_get_font_file_path

# 3) If there are top‐level constants that already called get_font_file_path at import time,
#    you can also blank them out:
try:
    _fonts_mod.PINGFANG_FONT_FILE_PATH = ""
    _fonts_mod.SIMFANG_FONT_FILE_PATH = ""
    ...
    def ocr(self, image_bytes: bytes) -> str:
        ...
        lines_data = [
            ...
            for poly, text, score in zip(
                single_result["rec_polys"],
                single_result["rec_texts"],
                single_result["rec_scores"]
            )
        ]
